...
100% │█
 ...
 12% │████████████████████████████████████         ██████
  ...
  이 과제는 어느 에피소드에서도 성공하지 않았습니다. 이 정책은 합성 불확실
  영역을 가진 관절 스윕이고, 큐브를 향해 뻗지 않으며 애초에 그럴 생각이
  없었습니다. 그러니 위의 하강은 기계장치가 작동한 것 - 인터럽트, 결정,
  회상 - 이지 정책이 무언가를 더 잘하게 된 것이 아닙니다.
```

**두 번째 숫자를 보세요.** 시도하기를 그만둔 정책도 **정확히 같은 하강선**을 그립니다.
바디가 큐브 높이를 보고하기 전까지는 그 둘을 구분할 방법이 없었고, 예제는 **하강만으로**
PASS를 찍었습니다. 이제 둘 다 잽니다. 이 정책에 대한 정직한 독해는 *고리가 돈다*이지
**절대 *고리가 배운다*가 아닙니다.** 실제 정책과 실제 조작자로 바꾸는 것이 v0.3
실험입니다.
...
스크립트입니다. LoRA 파인튜닝과 사람으로 바꾸는 것이 v0.3 실험입니다. 이건 v0.1이
기계장치를 실제로 닫는다는 증명이고, 둘을 흐리는 데모는 아무도 묻지 않은 질문에
답하면서 정작 중요한 질문에 답한 것처럼 보이게 만들기 때문에 분명히 적어둡니다.
...
`tendon calibrate`가 그 정책이 그 몸에서 내는 **전형적인 불일치**를 측정합니다. 그래야
신뢰도 점수를 읽을 척도가 생깁니다 — 없으면 로드된 체크포인트는 아무 점수도 보고하지
않고 스스로 인터럽트를 걸 수 없으며, 실행 전에 그렇게 말합니다.

**그건 척도이지 임계값이 아닙니다.** 그리고 그 차이가 실제로 물립니다. 기준이
중앙값이라, 임계값 0.5 — `skill.yaml`의 기본값 — 는 **예측 절반마다 사람을 부른다**는
뜻입니다. `smolvla_base` + 어댑터로 측정한 값:
...
어느 쪽이 옳은지는 측정으로 답할 수 없습니다. *부르지 않았을 때 무엇이 잘못되는가*에
달렸고, 그건 누군가 실제로 개입한 에피소드에서만 보입니다. ADR 0003이 그걸 v0.3에
둡니다. 표가 주는 건 **아무도 손에 쥔 적 없던 나머지 절반** — 각 선택이 얼마나 많은
중단을 대가로 하는가입니다.
...
어느 문으로 들어온 에피소드든 그 선에 들어갑니다. `tendon run`과 `tendon eval`도 셸과
똑같이 로그에 씁니다. **조작자가 없는 실행은 빠진 점이 아니라 교정 0에서의 개입률**이고,
나머지 모든 점은 그 왼쪽 끝을 기준으로 재집니다.
...
tendon calibrate <skill> 전형적인 불일치를 측정해 신뢰도에 척도를 부여
